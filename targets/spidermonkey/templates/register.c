...
static bool js_${current_class.underlined_class_name}_finalize(se::State& s)
{
    ...
    CCLOGINFO("jsbindings: finalizing JS object %p (${current_class.namespaced_class_name})", SE_THIS_OBJECT<${current_class.namespaced_class_name}>(s););
...
{
    CCLOGINFO("jsbindings: destory JS object %p (${current_class.namespaced_class_name})", current_class);
    #if $current_class.is_ref_class
    ${current_class.namespaced_class_name}* cobj = SE_THIS_OBJECT<${current_class.namespaced_class_name}>(s);
    cobj->release();
    #else
